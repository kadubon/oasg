...
def run_worker(
    contract: Contract, binding: TaskBinding, *, split: str, key: Any, now: int
) -> Source:
    ...
    if not binding.lease_start <= observed_at < min(binding.lease_end, contract.expires_at):
        raise ValueError("current lease or authority expired")
    if split not in {"training", "confirmation"}:
        raise ValueError("unknown execution split")
    if (split == "training" and observed_at > contract.cutoff) or (
        split == "confirmation" and observed_at <= contract.cutoff
    ):
        raise ValueError("actual execution clock violates registered freeze")
    ...
    if binding.policy_digest != digest({"variant": variant}) or binding.input_digest != digest(
        inputs
    ):
        raise ValueError("task input or policy not registered")
    completed = subprocess.run(
        [sys.executable, "-m", "oasg.collective.worker"],
        input=encoded({"inputs": inputs, "variant": variant}),
        capture_output=True,
        timeout=10,
        check=True,
        shell=False,
    ...
    if (
        raw["implementation"] != contract.implementation
        or implementation_digest() != contract.implementation
    ):
    ...
    payload = dict(
        schema_id="oasg.collective.execution.v1",
        ...
        external_effects=0,
        verification="positive" if correct else "negative",
        complete=True,
        cost_id="cost:" + binding.attempt,
        work=sum(m.probes for m in measurements) + contract.overhead,
    )
...
def compare(
    contract: Contract, baseline: Source, candidate: Source, public_key: bytes
) -> dict[str, Any]:
    ...
    if (
        before.binding.arm != "baseline"
        or after.binding.arm != "candidate"
        or before.split != after.split
        or before.binding.run != after.binding.run
    ...
    workload = WorkloadManifest(
        workload_id=contract.scope.study,
    ...
    for coordinate in ca.positive_evidence:
        ...
        witnesses.append(
            PositiveEvidenceWitness(
                witness_id=coordinate,
                coordinate_id=coordinate,
                evidence_hashes=[receipt_hash(candidate.model_dump())]
                + ([receipt_hash(ck.to_dict())] if is_klb else []),
                required_receipt_types=list(
                    REQUIRED_WITNESS_RECEIPTS["KLB_2" if is_klb else "dimension"]
                ),
    ...
    mutation = MutationPlan(
        ...
        from_grade=bl.dimensions["budget"],
        to_grade=ca.dimensions["budget"],
        patch=patch.to_dict(),
    ).to_dict()
    ...
    common: dict[str, Any] = dict(
        ...
        execution_receipt_hash=receipt_hash(candidate.model_dump()),
        trial_ledger_prefix_hash=ca.ledger_prefix_hash,
        trial_reducer_snapshot_hash=receipt_hash(ca.to_dict()),
    )
    shadow = ShadowResult(
        status="shadow_passed" if supported else "shadow_rejected",
        observed_coordinates=ca.dimensions,
