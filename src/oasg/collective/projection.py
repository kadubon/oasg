...
def project(contract: Contract, source: Source, public_key: bytes) -> dict[str, Any]:
    ...
    payload = observation_payload(
        ...
        action_grades=actions,
        protected_debt=dict.fromkeys(REQUIRED_DIMENSIONS, protected),
        proof_obligation_receipts=[
            {"coordinate": x, "status": "receipt_valid"} for x in coordinates
        ]
        if support["runner_execution_support"]
        else [],
        positive_evidence=[
            {"coordinate": x, "evidence_hash": receipt_hash(source.model_dump())}
    ...
    records = seal_records(
        [
            event_record(
                event_id=result.binding.attempt,
                workflow_id=contract.scope.study,
                component_id=contract.scope.worker,
    ...
    return {
        "schema_id": "oasg.collective.projection.v1",
        "contract": digest(contract),
