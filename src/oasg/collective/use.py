...
def execute_use(root: Path) -> dict[str, Any]:
    ...
    pending = next(row for row in view["entries"] if row["id"] == "cycle-two-use")
    ...
    def fresh() -> None:
        ...
        if current["revision"] != view["revision"] or current["unresolved"] != ["cycle-two-use"] or current["withdrawn"]:
        ...
        current_task(store,expected["run"],expected["ccr_trial"],contract.scope.worker,expected["token"],
                     expected_config=original_config)

    fresh()
    kernel = open_kernel(root/"memory",contract,trial,public_key,before_use=fresh)
    _, wire, _ = modules()
    ...
    cost = wire.Cost(id="cycle-two-work",stage="use",amount=str(len(text.splitlines())))
    outcome = runtime.use(wire.digest(q),text,identity="cycle-two",now=now,
        expected=runtime.store.inspect()["revision"],intent=intent,
        receipts=[memory["receipt"]["receipt_id"]],cost=cost)
    if not outcome["service"]:
        raise ValueError("native use did not verify")
    applied = loads((root/"memory"/"last-use.json").read_bytes())
    return {"retrieved":chosen[0],"outcome":outcome,"applied":applied,
            "receiver_B_eligible":False,"receiver_C_supported":False,
            "cost":cost.model_dump(),"process_id":__import__("os").getpid()}


if __name__ == "__main__":
    sys.stdout.buffer.write(encoded(execute_use(Path(sys.argv[1]))))
