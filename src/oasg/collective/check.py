...
from oasg.collective.wire import Contract, Source, ProjectionReport, digest, encoded, loads
...
def check_projection(
    contract: Contract, report: dict[str, Any], public_key: bytes
) -> dict[str, Any]:
    report = loads(encoded(report))
    ProjectionReport.model_validate(report)
    if report["contract"] != digest(contract):
    ...
    if verify_records(records).status != "ledger_prefix_valid":
    ...
    if (record["collector_id"], record["coverage_scope"], record["authority_scope"]) != (
        "local",
        "default",
        "local",
    ):
    ...
    metadata = {
        ...
        "ledger_profile_epoch": LEDGER_PROFILE_EPOCH,
        "schema_epoch": "oasg.event_record.v1",
        "policy_epoch": "oasg.policy.v1.0",
        "parent_event_ids": [],
    ...
    if any(digest(record[k]) != digest(v) for k, v in metadata.items()):
    ...
    supported = (
        result.evidence_class == contract.evidence_class == "executed_finite_software"
        and result.complete
    ...
    budget = (
        ("surplus" if result.work <= contract.efficient_work else "acceptable")
        if supported
        else "blocked"
    )
    snapshot = reduce_records(records)
    expected_dimensions = {
        x: budget if x == "budget" else expected_floor for x in REQUIRED_DIMENSIONS
    }
    expected_actions = {
        x: "blocked" if x in {"emit_claim", "promote_workflow"} else expected_floor
        for x in ACTION_CLASSES
    }
    if (
        snapshot.dimensions != expected_dimensions
        or snapshot.action_grades != expected_actions
    ...
    obligations = (
        [{"coordinate": x, "status": "receipt_valid"} for x in coordinates] if supported else []
    )
    witnesses = (
        [{"coordinate": x, "evidence_hash": receipt_hash(source.model_dump())} for x in coordinates]
        if supported
        else []
    )
    if (
        payload["payload_type"] != "observation"
        or payload["repair_receipts"]
        or payload["proof_obligation_receipts"] != obligations
        or payload["positive_evidence"] != witnesses
        or payload["dimensions"] != expected_dimensions
        or payload["action_grades"] != expected_actions
    ...
    if digest(payload["policy"]) != digest(
        {
            "effect_classes": ["pure"],
            "semantic_scope": "none",
    ...
    expected_evidence = (
        {
            x: [receipt_hash(source.model_dump())]
            for x in ["budget", *["KLB_2." + a for a in ACTION_CLASSES]]
        }
        if supported
        else {}
    )
    ...
    if digest(payload["model_event"]) != digest(
        {
    ...
    if digest(report["dimensions"]) != digest(
        {
...
def check_trial(contract: Contract, trial: dict[str, Any], public_key: bytes) -> dict[str, Any]:
    ...
    if (
        trial["schema_id"] != "oasg.collective.trial.v1"
        or trial["execution_authorization"] is not False
    ...
    if (left.binding.arm, right.binding.arm) != ("baseline", "candidate") or (
        left.binding.run != right.binding.run
        or left.split != right.split
        or left.binding.input_digest != right.binding.input_digest
        or left.binding.task != right.binding.task
        or left.binding.trial != right.binding.trial
    ...
    comparison = ComparisonContract(
        comparison_contract_id=digest(contract),
        workload_manifest_id=contract.scope.study,
        candidate_mutation_id="collective-routing",
    )
    if (
        digest(trial["comparison"]) != digest(comparison.model_dump(mode="json"))
        or workload.workload_id != contract.scope.study
    ):
        raise ValueError("unregistered comparison protocol")
    expected_inputs = [receipt_hash({"input": m.input}) for m in left.measurements]
    if workload.input_hashes != expected_inputs or workload.canonical_input_order != [
        str(i) for i in range(len(expected_inputs))
    ]:
        raise ValueError("trial inclusion changed")
    gate = evaluate_gate(
        before,
        after,
        calculate_klb(before),
        calculate_klb(after),
        ComparisonContract.model_validate(trial["comparison"]),
        workload,
        [PositiveEvidenceWitness.model_validate(w) for w in trial["witnesses"]],
    )
    if digest(gate.to_dict()) != digest(trial["gate"]) or digest(trial["work"]) != digest(
        {"baseline": left.work, "candidate": right.work}
    ):
    ...
    patch = MutationPatch(
        mutation_id="collective-routing",
        op="set_routing_policy",
        target_action_id="local_reversible",
        coordinate_id="budget",
        value=contract.candidate,
        mutator_id="registered-finite-routing",
        precondition_policy_hash=contract.library_digest,
    )
    mutation = MutationPlan(
        mutation_id=patch.mutation_id,
        target_component_id=contract.scope.worker,
        coordinate_id="budget",
        action_id="local_reversible",
        from_grade=before.dimensions["budget"],
        to_grade=after.dimensions["budget"],
        patch=patch.to_dict(),
    ).to_dict()
    if digest(mutation) != digest(trial["mutation"]):
    ...
    common: dict[str, Any] = dict(
        mutation_id=patch.mutation_id,
        ledger_prefix_hash=after.ledger_prefix_hash,
    ...
    shadow = ShadowResult(
        status="shadow_passed" if supported else "shadow_rejected",
        observed_coordinates=after.dimensions,
        replayed_event_count=1,
        **common,
    )
    lease = LeaseResult(
        status="lease_passed" if supported else "lease_rejected_cap_exceeded",
        max_events=1,
        executed_event_count=1,
        effect_counts={"external": 0},
        resources={"work": right.work},
        rollback_available=True,
        **common,
    )
    if (
        digest(trial["shadow"]) != digest(shadow.to_dict())
        or digest(trial["lease"]) != digest(lease.to_dict())
        or digest(trial["active"])
        != digest(active_promotion_receipt(gate, shadow, lease, mutation))
    ):
