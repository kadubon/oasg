...
def emit(action: Callable[[], Any]) -> None:
    try:
        ...
        sys.stdout.write(encoded({"ok":False,"status":"rejected","reason":str(exc)[:1000],
            "execution_authorization":False}).decode()+"\n")
...
def inspect_command(path: Annotated[Path,typer.Argument()]) -> None:
    ...
    emit(lambda: {"ok":True,"contract":Contract.model_validate(loads(path.read_bytes())).model_dump(),
...
def project_command(contract: Path, source: Path, public_key: str) -> None:
    ...
    emit(lambda:project(Contract.model_validate(loads(contract.read_bytes())),
                        Source.model_validate(loads(source.read_bytes())),bytes.fromhex(public_key)))
...
def check_command(contract: Path, report: Path, public_key: str) -> None:
    ...
    def operation() -> Any:
        ...
        checker = check_trial if value.get("schema_id") == "oasg.collective.trial.v1" else check_projection
...
@app.command("example")
def example_command(out: Annotated[Path,typer.Option("--out")],
                    execute: Annotated[bool,typer.Option("--execute")]=False) -> None:
    ...
    def operation() -> Any:
        ...
        result = example(out)
        return {"ok":result["ok"],"gate":result["gate"],"active":result["active"],
            "work":result["work"],"second_cycle_policy":result["second_cycle"]["applied"]["policy"],
            "after_withdrawal_eligible":result["withdrawal"]["memory_eligible"],
            "accounting":result["accounting"]["status"],"report":str(out/"report.json"),
            "evidence_class":result["evidence_class"],"operationally_observed":False}
    emit(operation)
