"""Runner-produced trial ledgers for strong-baseline adaptive readiness."""
    ...
    *,
    ...
    for task in tasks:
        baseline_rows.append(
            run_task(
                task=task,
                condition="strong_trial_baseline",
                config=config,
        ...
        candidate_rows.append(
            run_task(
                task=task,
                condition="strong_trial_candidate",
                config=config,
    ...
    write_history(baseline_ledger, "strong_trial_baseline", baseline_rows)
    write_history(candidate_ledger, "strong_trial_candidate", candidate_rows)
    ...
    receipt = {
        "receipt_type": "strong_trial_bundle_receipt",
        ...
        "positive_evidence": [
            {
                "coordinate": _coordinate_for_policy(candidate_policy_id),
...
def _coordinate_for_policy(policy_id: str) -> str:
    if "retry" in policy_id:
        return "KLB_2.close_obligation"
    if "receipt" in policy_id or "validator" in policy_id:
