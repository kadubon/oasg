"""Shared helpers for the decisive OASG effect protocol."""
...
def classify_decisive(
    *,
    ...
    if policy_qualification.get("status") != "policy_catalog_qualified":
        return "workload_not_sensitive"
    if promotion_qualification.get("status") != "promotion_mechanism_qualified":
    ...
    meaningful_delta = -int(
        round(baseline_auc * int(config.get("minimum_meaningful_reduction_bps", 1500)) / 10000)
    )
    if (
        effect_bps >= int(config.get("effect_confirmed_min_reduction_bps", 2000))
        and ci_upper < meaningful_delta
    ):
