"""Deterministic task generator for the decisive OASG experiment."""
    ...
    *,
    ...
    for epoch in range(1, epoch_count + 1):
        ...
        for index, family in enumerate(families[:tasks_per_epoch], start=1):
            task = _task_for_family(seed, epoch, index, phase, burst, family, rng)
            tasks.append(task)
    return tasks
...
def _burst_for_epoch(epoch: int, warmup_epochs: int) -> str:
    if epoch <= warmup_epochs:
        return ("warmup_format_drift", "warmup_schema_gap", "warmup_retry_gap")[
            (epoch - 1) % 3
        ]
    cycle = (epoch - warmup_epochs - 1) % 6
    return (
        "validator_failure_burst",
        "context_budget_pressure",
        "obligation_backlog_burst",
        "stale_format_drift",
        "rollback_receipt_gap",
        "recovery",
    )[cycle]
...
def _task_for_family(
    ...
    task_id = f"decisive_s{seed}_e{epoch:02d}_t{index:02d}"
    base: dict[str, Any] = {
        "task_id": task_id,
        "seed": seed,
        "epoch": epoch,
    ...
    if family == "code_transform":
        snake = f"task_{epoch}_{index}"
        expected = {"identifier": snake}
        return {
            **base,
