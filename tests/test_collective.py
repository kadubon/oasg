...
def contract_key(**changes):
    ...
    now = int(time.time())
    raw = dict(scope=Scope(mission="finite", study="study", worker="worker", receiver="A",
        context="finite", namespace="disposable"), training=["z\na", "c\nb"],
        confirmation=["z\ny\nx\nw\nv\nu\nt\ns"],
        evidence_class="executed_finite_software", registered_at=now-2, cutoff=now-1,
        expires_at=now+600, max_work=100, efficient_work=12, overhead=2,
        verification_budget=4, cleanup_budget=1, pool="pool", ccr_registration="0"*64,
        ccr_revision=0, library_digest=receipt_hash(WorkflowPolicyState.default().to_dict()),
...
def sources(contract, key):
    ...
    for arm, variant in [("baseline", "scan"), ("candidate", contract.candidate)]:
        binding = TaskBinding(run="run", task="task", trial="trial", attempt=arm,
            worker="worker", receiver="A", arm=arm, fencing_token=1, revision=1,
            reservation="reserve", input_digest=digest(contract.confirmation),
            policy_digest=digest({"variant": variant}), lease_start=now, lease_end=now+300)
...
@pytest.mark.parametrize("changes", [
    {"evidence_class": "synthetic"}, {"candidate": "scan"},
    {"candidate": "skip-last"}, {"overhead": 90}, {"efficient_work": 5},
])
