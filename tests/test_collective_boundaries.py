...
def test_source_authentication_and_bytes(evidence):
    ...
    with pytest.raises(ValueError, match="signature"):
        checked_execution(
            contract,
            Source(original=raw.decode(), sha256=sha(raw), producer_digest=digest(altered)),
            public,
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda r: r.update(contract="0" * 64),
        lambda r: r.update(records=[]),
        lambda r: r.update(field_map={}),
...
@pytest.mark.parametrize(
    ...
    [
        lambda r: r.update(extra=True),
        lambda r: r.update(collector_id="remote"),
        lambda r: r.update(event_id="other-attempt"),
        lambda r: r.update(parent_event_ids=["foreign"]),
        lambda r: r["payload"].update(extra=True),
        lambda r: r["payload"].update(repair_receipts=[{}]),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda t: t.update(execution_authorization=True),
        lambda t: t.update(collective_benefit=1),
        lambda t: t.update(contract="0" * 64),
        lambda t: t["projections"].reverse(),
        lambda t: t["work"].update(candidate=0),
...
def test_worker_boundaries(text, variant):
    with pytest.raises(ValueError):
...
@pytest.mark.parametrize(
    ...
    [
        [],
        {"inputs": ["x"], "variant": "indexed", "command": "inert"},
        {"inputs": [], "variant": "indexed"},
        {"inputs": "text", "variant": "indexed"},
    ],
)
def test_worker_stdin_is_only_bounded_data(monkeypatch, wire_request):
    import io
    from types import SimpleNamespace
    from oasg.collective import worker

    monkeypatch.setattr(
        worker.sys, "stdin", SimpleNamespace(buffer=io.BytesIO(encoded(wire_request)))
    )
    with pytest.raises(ValueError):
        worker.main()


def test_inert_text_has_no_expression_or_url_interpretation(tmp_path):
    text = 'https://example.invalid/\n__import__("os").system("echo forbidden")\n../outside'
    assert execute(text, "indexed").output == reference(text)
    assert list(tmp_path.iterdir()) == []


def test_confirmation_freeze_and_variant_binding(evidence):
    contract, key, public, pair = evidence
    before = signed(
        pair[1],
        key,
        lambda v: v.update(
            observed_at=contract.cutoff,
            received_at=contract.cutoff,
            binding=v["binding"] | {"lease_start": contract.cutoff},
        ),
    )
    with pytest.raises(ValueError, match="freeze"):
        checked_execution(contract, before, public)
    other = signed(
        pair[1],
        key,
        lambda v: v.update(
            variant="scan", binding=v["binding"] | {"policy_digest": digest({"variant": "scan"})}
        ),
    )
    with pytest.raises(ValueError, match="variant"):
        checked_execution(contract, other, public)


def test_json_types_do_not_share_boolean_numeric_equality(evidence):
    contract, _, public, pair = evidence
    for edit in (
        lambda r: r["payload"]["policy"].update(workflow_promotion_authorized=0),
        lambda r: r["payload"]["policy"].update(claim_emitting=0),
    ):
        result = project(contract, pair[1], public)
        edit(result["records"][0])
        result["records"] = seal_records(result["records"])
        with pytest.raises(ValueError):
            check_projection(contract, result, public)


def test_compare_refuses_cross_run_pair(evidence):
    contract, key, public, pair = evidence
    other = signed(pair[1], key, lambda v: v["binding"].update(run="foreign"))
    with pytest.raises(ValueError, match="unpaired"):
        compare(contract, pair[0], other, public)


def test_worker_replacement_during_execution(evidence, monkeypatch):
    from types import SimpleNamespace
    from oasg.collective import trial

    contract, key, _, pair = evidence
    binding = pair[1].execution().binding
    monkeypatch.setattr(
        trial.subprocess,
        "run",
        lambda *args, **kwargs: SimpleNamespace(
            stdout=encoded({"implementation": "0" * 64, "rows": []})
        ),
    )
    with pytest.raises(ValueError, match="changed during"):
        trial.run_worker(contract, binding, split="confirmation", key=key, now=binding.lease_start)


@pytest.mark.parametrize("mode", ["expired", "unknown", "training", "confirmation"])
def test_actual_worker_clock_cannot_be_retrodated(evidence, monkeypatch, mode):
    from oasg.collective import trial

    contract, key, _, pair = evidence
    binding = pair[1].execution().binding
    actual = binding.lease_end if mode == "expired" else binding.lease_start
    if mode == "confirmation":
        contract = contract.model_copy(update={"cutoff": actual})
    monkeypatch.setattr(trial.time, "time", lambda: actual)
    split = "unknown" if mode == "unknown" else "training" if mode == "training" else "confirmation"
    with pytest.raises(ValueError, match="expired|split|freeze"):
        trial.run_worker(contract, binding, split=split, key=key, now=binding.lease_start)
