...
def test_packaged_additive_schemas():
    for name, schema in schema_documents().items():
        resource = files("oasg.collective").joinpath("resources/collective-"+name+".schema.json")
...
def test_cli_read_and_check(tmp_path):
    ...
    cpath, spath, rpath = [tmp_path / name for name in ("contract.json", "source.json", "report.json")]
    ...
    result = runner.invoke(app, ["collective", "project", str(cpath), str(spath), "invalid-public-key"])
...
def test_cli_no_implicit_initialization(tmp_path):
    ...
    for arguments in (["status", str(root)], ["replay", str(root)],
                      ["example", "--out", str(root)], ["reconcile", str(root)]):
