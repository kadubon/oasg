...
@pytest.mark.parametrize("change", [
    {"ccr_registration":"0"*64}, {"verifier_key":"0"*64}, {"cutoff":0},
    {"ccr_revision":1}, {"pool":"other"}, {"cleanup_budget":0},
    {"verification_budget":1}, {"max_work":501}, {"candidate":"scan"},
    {"evidence_class":"synthetic"}, {"implementation":"0"*64}, {"checker":"0"*64},
])
def test_registration_constraints(completed, change):
    ...
    with pytest.raises(ValueError):
        ccr_bridge.check_registration(contract.model_copy(update=change), config,
            bytes.fromhex(registration["public_key"]), now=contract.registered_at)
...
def test_registration_cannot_widen_native_authority(completed):
    ...
    with pytest.raises(ValueError, match="deadline"):
        ccr_bridge.check_registration(contract.model_copy(update={"ccr_registration":digest(earlier)}),
            earlier, public, now=contract.registered_at)


def test_ccr_real_lease_contention_and_stale_fence(tmp_path):
    _, _, public = contract_key()
    config = ccr_bridge.registration(public,["z\na"],study="contended",worker="worker",
        context="finite",artifact="1"*64)
    ...
    def claim(_):
        try:
            return engine.claim(store,run_id,trial["trial_id"],worker="worker",ttl_seconds=30)["ok"]
        except ValueError:
            return False
    ...
    assert ccr_bridge.current_task(store,run_id,trial["trial_id"],"worker",token,expected_config=config)
    with pytest.raises(ValueError):
    ...
    with pytest.raises(ValueError, match="registration"):
        ccr_bridge.current_task(store,run_id,trial["trial_id"],"worker",token,expected_config=changed)
...
def test_expired_or_changed_worker_never_executes(monkeypatch):
    ...
    with pytest.raises(ValueError,match="registered"):
        run_worker(contract.model_copy(update={"implementation":"0"*64}),binding,
            split="confirmation",key=key,now=int(time.time()))
    with pytest.raises(ValueError,match="expired"):
        run_worker(contract,binding,split="confirmation",key=key,now=binding.lease_end)
    with pytest.raises(ValueError,match="policy"):
        run_worker(contract,binding.model_copy(update={"policy_digest":"0"*64}),
            split="confirmation",key=key,now=int(time.time()))
