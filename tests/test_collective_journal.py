...
def registered(tmp_path):
    ...
    revision = journal.append(
        "register",
        "registration",
        {"contract": contract.model_dump(), "public_key": public.hex()},
        expected="0" * 64,
    )
...
def test_idempotent_delivery_and_conflicts(tmp_path):
    ...
    with pytest.raises(ValueError, match="unregistered"):
        journal.require_ready(
            contract.model_copy(update={"pool": "other"}), now=contract.registered_at
        )
    with pytest.raises(ValueError, match="immutable"):
        journal.append(
            "register", "replacement", {"contract": contract.model_dump()}, expected=final
        )
...
def test_history_tampering(tmp_path):
    ...
    for changes in ({"previous": "1" * 64}, {"extra": 1}, {"data": []}):
        ...
        with pytest.raises(ValueError):
            journal.inspect()
    for kind, data in [
        ("ack", {"intent": "absent", "original_result": "{}", "result_sha256": digest({})}),
        ("unknown", {}),
        ("intent", {}),
    ]:
